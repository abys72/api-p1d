...
class TestAcceptanceAccessToken:
    ...
    async def test_access_token_response_user_incorrect(self, auth_service: LoginUserService) -> None:
        with patch.object(auth_service._user_repository, 'find_by_credentials',
                          new_callable=AsyncMock) as mock_find_by_credentials:
            mock_find_by_credentials.return_value = False
            token_request: TokenRequest = TokenRequest(username="Test1", password="Error")
            token_params: TokenResponse | None = await auth_service.get_token(request=token_request,
                                                                              secret_key="secret")
            assert token_params is None

    async def test_access_token_input_correct(self, auth_service: LoginUserService) -> None:
        with patch.object(auth_service._user_repository, 'find_by_credentials',
                          new_callable=AsyncMock) as mock_find_by_credentials:
            mock_find_by_credentials.return_value = True
            with patch(
                    'app.modules.v1.endpoints.auth.access_token.login_user_service.JWTHelper.create_access_token') as mock_create_token:
                expected_token_dict = {
                ...
                mock_create_token.assert_called_once_with(identity="admin", secret_key="secret")
