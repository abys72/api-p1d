...
class TestIntegrationProductRecommendation:
    ...
    async def test_create_and_get_product_recommendation(self, mongo_session) -> None:
        ...
        service: ProductRecommendationService = container.product_recommendation_service()
        await service.create_subsequence(sequence=input_sequence)

        recommendations: List[ProductRecommendationModel] | None = await service.get_product_recommendation()
        ...
        assert len(recommendation.subsequences_generated) == len(expected_subsequences_generated)
