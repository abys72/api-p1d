...
def global_jwt_check():
    ...
    excluded_paths = ["/api/v1/auth/access-token"]
    ...
    try:
        decoded = JWTHelper.decode_access_token(
            token, secret_key=current_app.config.get("SECRET_KEY")
        )
        request.jwt_payload = decoded
        logger.info(
            f"Token validated successfully for user {decoded.get('sub', 'unknown')}"
        )
