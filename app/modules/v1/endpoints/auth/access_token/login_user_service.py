...
class LoginUserService:
    ...
    async def get_token(
        self, request: TokenRequest, secret_key: str
    ) -> TokenResponse | None:
        self._logger.info(f"Login attempt for username: {request.username}")
        is_valid_user: bool = await self._user_repository.find_by_credentials(
            request.username, request.password
        )
        if is_valid_user:
            token_params: Dict[str, str | int] = JWTHelper.create_access_token(
                identity=request.username, secret_key=secret_key
            )
