...
from app.modules.v1.endpoints.auth.access_token.protocol.login_user_query_protocol import LoginUserQueryProtocol
...
class LoginUserRepository:
    ...
    def __init__(self, logger: AppLogger, login_user_query: LoginUserQueryProtocol) -> None:
        self.__logger = logger
        self.__login_user_query = login_user_query

    async def find_by_credentials(self, username: str, password: str) -> bool:
        try:
            user: User | None = await self.__login_user_query.find_by_credentials(username, password)
            if user:
