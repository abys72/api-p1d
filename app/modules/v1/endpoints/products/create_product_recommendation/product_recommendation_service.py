...
class ProductRecommendationService:
    ...
    def __init__(
        self,
        product_recommendation_repository: ProductRecommendationRepository,
        logger: AppLogger,
    ) -> None:
    ...
    async def create_subsequence(self, sequence: List[int]) -> None:
        try:
            ...
            await self.__product_recommendation_repository.insert_product_recommendation(
                recommendation_model
            )
        except Exception as e:
            self.__logger.error(
                f"Error while creating subsequence for sequence '{sequence}': '{e}'."
            )
            raise e
        return None
    ...
    async def get_product_recommendation(
        self,
    ) -> List[ProductRecommendationModel] | None:
        try:
            recommendations_models: List[ProductRecommendationModel] = (
                await self.__product_recommendation_repository.get_product_recommendations()
            )
            if recommendations_models is None or len(recommendations_models) == 0:
                ...
                return None
            recommendations_models_with_original: List[ProductRecommendationModel] = (
                self._find_subsequence(subsequence=recommendations_models)
            )
    ...
    def _create_subsequences(self, sequence: List[int]) -> List[List[int]] | None:
        ...
        try:
            ...
            sorted_subsequences: List[List[int]] = sorted(
                result_subsequence, key=lambda s: (len(s), s)
            )
        except Exception as e:
    ...
    def _find_subsequence(
        self, subsequence: List[ProductRecommendationModel]
    ) -> List[ProductRecommendationModel]:
        for sequence in subsequence:
            original_subsequence: List[int] = max(
                sequence.subsequences_generated, key=len
            )
