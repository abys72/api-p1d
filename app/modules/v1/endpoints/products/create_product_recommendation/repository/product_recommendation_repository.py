...
from app.modules.v1.endpoints.products.create_product_recommendation.protocols.product_recommendation_query_protocol import \
    InsertProductRecommendationQueryProtocol, GetProductRecommendationQueryProtocol
...
class ProductRecommendationRepository:
    ...
    __insert_product_recommendations_query_protocol: InsertProductRecommendationQueryProtocol
    __get_product_recommendations_query_protocol: GetProductRecommendationQueryProtocol

    def __init__(self, logger: AppLogger, product_recommendation_mapper: ProductRecommendationMapper,
        ...
        self.__insert_product_recommendations_query_protocol = insert_product_recommendations_query_protocol
        self.__get_product_recommendations_query_protocol = get_product_recommendations_query_protocol

    async def insert_product_recommendation(self, recommendation_ids: ProductRecommendationModel) -> None:
        self.__logger.info(f"Inserting product recommendations: '{recommendation_ids}'.")
        odm_recommendation_model: Subsequence = self.__product_recommendation_mapper.map_insert_subsequences(
    ...
    async def get_product_recommendations(self) -> List[ProductRecommendationModel]:
        ...
        list_of_subsequence: List[
            Subsequence] = await self.__get_product_recommendations_query_protocol.get_subsequences()
        list_product_recommendations: List[
            ProductRecommendationModel] = self.__product_recommendation_mapper.map_get_subsequences(
