    ...
    async def post(self) -> ApiResponse:
        try:
            ...
            if not isinstance(sequence, list):
                return CustomResponse.bad_request(message=f"Sequence must be a list '{type(sequence).__name__}'.")

            if len(sequence) == 0:
                return CustomResponse.bad_request(message="Sequence cannot be empty")

            if not all(isinstance(x, int) for x in sequence):
                return CustomResponse.bad_request(message=f"Sequence must contain only integers '{sequence}'.")

            if any(x < 0 for x in sequence):
                return CustomResponse.bad_request(message=f"Sequence cannot contain negative integers '{sequence}'")
