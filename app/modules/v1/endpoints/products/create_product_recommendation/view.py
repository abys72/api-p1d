    ...
    async def post(self) -> ApiResponse:
        try:
            ...
            parser.add_argument(
                ...
                required=True,
                location="json",
                help="sequence is mandatory",
            )
            args = parser.parse_args()
            sequence: List = args.sequence
            if not isinstance(sequence, list):
                return CustomResponse.bad_request(
                    message=f"Sequence must be a list '{type(sequence).__name__}'."
                )

            if len(sequence) == 0:
                return CustomResponse.bad_request(message="Sequence cannot be empty")

            if not all(isinstance(x, int) for x in sequence):
                return CustomResponse.bad_request(
                    message=f"Sequence must contain only integers '{sequence}'."
                )

            if any(x < 0 for x in sequence):
                return CustomResponse.bad_request(
                    message=f"Sequence cannot contain negative integers '{sequence}'"
                )
            ...
            return CustomResponse.ok(
                data={"sequence": sequence}, message="Subsequence created successfully"
            )
        except Exception as e:
            return CustomResponse.server_error(
                message=f"An error occurred while creating subsequence. '{str(e)}'."
            )
    ...
    async def get(self) -> ApiResponse:
        try:
            ...
            recommendations: List[ProductRecommendationModel] | None = (
                await product_recommendation_service.get_product_recommendation()
            )

            if recommendations is None:
                return CustomResponse.ok(
                    message="No recommendations found", data={"recommendations": []}
                )
